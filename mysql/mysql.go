...
func ConnectMysqlDemo( connector apiserverclient.Resource ) {
	...
	config := &ssh.ClientConfig{
		...
		Auth: []ssh.AuthMethod{
			ssh.Password(connector.SshPassword),
		},
		HostKeyCallback: ssh.InsecureIgnoreHostKey(),
	}

	conn, err := ssh.Dial("tcp", connector.SshHost+":"+connector.Port, config)
	if err != nil {
	...
	*/
	//session.Run("mysql")
	session.Run("mysql -h localhost -u root")
